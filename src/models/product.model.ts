...
class Product {
  ...
  async createProduct(product: ProductType): Promise<ProductReturnType> {
    try {
      ...
      throw new Error(
        `Unable to create (${product.name}) cuz ${(err as Error).message}`
      );
    }
  }
  ...
  async showProducts(): Promise<ProductReturnType[]> {
    try {
      const connection = await db.connect();
      const sql = `SELECT id, name, price FROM ${this.table}`;
      const result = await connection.query(sql);
      connection.release();
      return result.rows;
    } catch (err) {
      throw new Error(`Error showing products cuz ${(err as Error).message}`);
    }
  }

  async getProduct(productName: string): Promise<ProductReturnType> {
    try {
      const connection = await db.connect();
      const sql = `SELECT id, name, price FROM ${this.table} WHERE name=$1`;
      const result = await connection.query(sql, [productName]);
      connection.release();
      return result.rows[0];
    } catch (err) {
      throw new Error(
        `Error showing the product ${productName} cuz ${(err as Error).message}`
      );
    }
  }
  ...
  async deleteProduct(id: number): Promise<ProductReturnType> {
    try {
      ...
      await connection.query(`ALTER SEQUENCE products_id_seq RESTART WITH ${id}`);
      connection.release();
      return result.rows[0];
    } catch (err) {
      throw new Error(
